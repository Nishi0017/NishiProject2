    ...
    private void Update()
    {
        ...
        foreach (Collider objectInCollider in objectsInCollider)
        {
            ...
            if ((searchAngle * -0.5f <= angleToEnemy || angleToEnemy <= searchAngle * 0.5f) && !IsOtherObjectBetween(objectInCollider.gameObject.transform))
            {
                ...
                {
                    Debug.Log("add:" + objectInCollider.gameObject);
        ...
        foreach (GameObject enemy in visibleEnemies)
        {
            ...
            Debug.Log("角度：" + (searchAngle * -0.5f <= angleToEnemy || angleToEnemy <= searchAngle * 0.5f));
            Debug.Log("距離：" + (distanceToEnemy <= searchRadius * searchRadius));
            Debug.Log("壁：" + !IsOtherObjectBetween(enemy.transform));
